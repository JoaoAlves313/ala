...
import { Search, GraduationCap, Youtube, Crown, Library, X, Plus, Trash2, ExternalLink, Folder } from 'lucide-react';
    ...
    <div className="flex h-screen bg-[#0f0f0f] text-white font-sans selection:bg-red-500 selection:text-white overflow-hidden">
      ...
      <div className="w-72 bg-[#121212] border-r border-[#222] flex flex-col h-full flex-shrink-0 z-20 shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-1">
          <h2 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3 px-3 mt-2">Libraries</h2>
          ...
          {categories.map(cat => (
            ...
            >
              <div className="flex items-center gap-3 truncate">
                <Folder className={`w-4 h-4 ${activeCategoryId === cat.id ? 'text-red-500' : 'text-gray-500'}`} />
                <span className="truncate">{cat.name}</span>
              </div>
              <span className={`text-xs px-2 py-0.5 rounded-full ${
                activeCategoryId === cat.id ? 'bg-red-600/20 text-red-400' : 'bg-[#222] text-gray-500 group-hover:bg-[#333]'
              }`}>
                {cat.links.length}
              </span>
            </button>
